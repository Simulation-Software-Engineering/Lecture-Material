...
- Submit your documentation website via an issue. You cannot submit a pull request since you cannot use a fork in the exercise. You can also add the issue before you are done in order to ask questions. In that case please mention us in the issue such that we get notified and add the label "work in progress" to show that the issue is not finished yet. Please add the label "finished" to the issue once you are finished and the issue is ready for review. You can find the labels on the right hand side when creating the issue on GitHub. You can also add/change the labels after the issue has been created.
